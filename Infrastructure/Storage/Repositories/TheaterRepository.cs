...
public class TheaterRepository(AppDbContext dbContext) : ITheaterRepository
{
    public async Task<List<Theater>> GetAllTheaters()
    {
        List<TheaterEntity> theaters = await dbContext
            .Theaters.Include(t => t.Rooms)
    ...
    {
        TheaterEntity? theater = await dbContext
            .Theaters.Include(t => t.Rooms)
            .ThenInclude(r => r.Seats)
            .Where(t => t.Id == id)
            .FirstOrDefaultAsync();
